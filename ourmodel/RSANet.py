...
class BasicBlockD(nn.Module):
    def __init__(self,
        ...
        self.conv2 = RSAConvDropoutNormReLU(conv_op, output_channels, output_channels, kernel_size, 1, conv_bias, norm_op,
...
class BottleneckD(nn.Module):
    def __init__(self,
        ...
        self.conv2 = ConvDropoutNormReLU(conv_op, bottleneck_channels, bottleneck_channels, kernel_size, stride,
                                         conv_bias,
                                         norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)
        self.conv3 = RSAConvDropoutNormReLU(conv_op, bottleneck_channels, output_channels, 1, 1, conv_bias, norm_op,
                                         norm_op_kwargs, None, None, None, None)
...
class ResidualUNet(nn.Module):
    def __init__(self,
        ...
        assert len(n_conv_per_stage) == n_stages, "n_blocks_per_stage must have as many entries as we have " \
                                                  f"resolution stages. here: {n_stages}. " \
                                                  f"n_blocks_per_stage: {n_conv_per_stage}"
    ...
    def __init__(self,
        ...
        for s in range(n_stages):
            ...
            stage = StackedResidualBlocks(
                n_conv_per_stage[s], conv_op, input_channels, features_per_stage[s], kernel_sizes[s], stride_for_conv,
...
class UNetResDecoder(nn.Module):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            sf_er.append(EdgeAwareREAM_Lite(input_features_below, stride_for_transpconv))
            sf_decoders.append(SF_Decoder(input_features_skip))
        ...
        self.sf_er = nn.ModuleList(sf_er)
        self.sf_decoders = nn.ModuleList(sf_decoders)
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            b = self.sf_er[s](skips[-(s + 1)], skips[-(s + 2)])
            x = self.sf_decoders[s](x, b)
